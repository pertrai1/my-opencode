...
3. **Get apply instructions**
   ...
   **Handle states:**
   - If `state: "blocked"` (missing artifacts): show the message and suggest `/openspec-continue-change`. If that command is not installed, run `openspec status --change "<name>" --json` to find the next artifact, then run `openspec instructions <artifact-id> --change "<name>" --json` for creation instructions.
