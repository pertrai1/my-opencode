...
description: Use only when the user explicitly invokes the OpenSpec fast-forward workflow.
...
In this configuration, route `/opsx-ff` through `sdlc-orchestrator`, which
owns lifecycle decisions and planning delegation. Do not use the direct steps
below when the orchestrator is available.
