...
- [x] 2.2 Add direct physical-line verification and TypeScript AST inspection for explicit `any` and `unknown` types without requiring target-repository ESLint configuration; verify fixture source files fail each threshold.
...
- [x] 4.3 Run `npm run typecheck`, `npm run lint`, `npm test`, and the full quality command against this repository; verify the resulting report is valid JSON and record any environment-dependent checks that cannot run. The full quality command returns findings for existing source that exceed the configured gates.
