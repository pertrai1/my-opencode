...
- **NEVER commit directly to the `main` or `master` branch. No exceptions.** When a commit is authorized, create a feature branch; push and open a pull request only when authorized.
...
- Before editing, inspect the affected code and the instructions needed for the task. For substantial changes, also inspect relevant tests and current `git status`/diff.
...
- Continue through implementation and the narrowest relevant verification; fix failures caused by the change. State explicitly if verification could not run or fails.
...
- Read the applicable repository instructions and documentation needed for the task; nearer instructions override broader ones.
