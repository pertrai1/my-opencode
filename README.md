...
Run `node ~/.config/opencode/scripts/checks-runner.mjs` from a target Node worktree to run its explicitly declared `typecheck`, `lint`, and `test` package scripts in that order. It selects `npm`, `pnpm`, `yarn`, or `bun` from `packageManager` or exactly one supported lockfile; it never installs dependencies or infers fallback commands. Use `--target <directory>` to select another worktree and `--timeout <seconds>` to override the 1,800-second per-stage limit. It writes paired durable artifacts to the target worktree at `.agents/reports/checks-<RUN_ID>.{json,md}`. Exit `0` means all configured stages passed; `1` is a failed or blocked run; `2` is an invocation, setup, process, or report error. Reports contain bounded, best-effort-redacted diagnostics and Git state before/after execution. They are useful memory for later agents or harnesses, but are historical evidence unless their recorded repository state still matches; rerun before claiming current verification. Add `.agents/reports/` to a target repository's ignore rules if that repository does not already ignore generated agent artifacts. `/checks` runs the same workflow. The runner is only a baseline: agents must still run checks required by target instructions or the source of truth.
...
- **Oracle & Do-Nothing Verifier Standards** — Verification workflows require establishing clean baseline preflight checks (Oracle Verifier) to isolate pre-existing repo issues and proving newly authored functional checks fail on the pre-change baseline (Do-Nothing Verifier) to prevent vacuous passes.
