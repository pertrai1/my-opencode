...
const secretScanPluginModule = require("../plugins/secret-scan.ts");
const secretScanPlugin =
  secretScanPluginModule.default ?? secretScanPluginModule;
const {
  buildScanArgs,
  buildToastMessage,
...
test("secret scan distinguishes leak findings from scan failures", () => {
  ...
  assert.strictEqual(hasGitleaksFindings("{}"), false);
  assert.strictEqual(
    hasGitleaksFindings(JSON.stringify([{ RuleID: "generic-api-key" }])),
    true,
  );
});
...
test("secret scan filters findings from gitignored files", async () => {
  ...
  const filtered = await filterIgnoredFindings(
    findings,
    async (filePath) => filePath === "service.json",
  );
