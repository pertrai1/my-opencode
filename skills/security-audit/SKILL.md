...
- **Output directory**: a new writable directory outside the target, defaulting to `~/.config/opencode/.agents/docs/security-audits/<repo-name>/run-<N>`, where `<N>` is the next unused integer. This follows the local OpenCode configuration's agent-artifact convention. Use a directory inside the target only when the user explicitly selects it and the parent verifies that version control ignores the whole directory. Otherwise stop and request an external path.
