...
When Phase 0 was skipped, the Phase 1 handoff must explicitly say `no-contract mode`, name the public entrypoint under test, and state the allowed API source of truth for that slice. The test-author may then derive the test-facing signature from that public evidence instead of a published contract file.
