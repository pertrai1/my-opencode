...
- `Functional Check`: identify at least one direct behavior hit that supports the intended outcome, at least one clean-pass or no-regression check when applicable, and baseline sensitivity ("Do-Nothing" check): confirm that newly authored functional or verification checks fail against the pre-change baseline commit (`git merge-base` or unmodified workspace) so that passes are not vacuous or tautological. If a new check passes on the unmodified baseline, flag it as a vacuous pass. If not applicable or unavailable, mark it explicitly as `unverified` and explain why.
